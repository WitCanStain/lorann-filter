...
if __name__ == "__main__":
    ...
    c_lib.fast_filter_wrapper_profiled.argtypes = ctypes.POINTER(ctypes.c_int), ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p
...
experiment parameters:\nn_repeat_runs = {n_repeat_runs}\nn_query_indices = {n_query_indices}\nquery_indices = {query_indices}\n\n")
    ...
    for i in range(n_repeat_runs):
        avg_recall = c_lib.fast_filter_wrapper_profiled(arr.ctypes.data_as(ctypes.POINTER(ctypes.c_int)), len(arr), k, clusters_to_search, points_to_rerank, ctypes.c_char_p(filter_attribute_b_string), ctypes.c_char_p(filter_approach_b_string), ctypes.c_char_p(exact_search_approach_b_string))
