...
EXPORT_SYMBOL int fast_filter_wrapper_profiled(int* idxs, int n_idxs, int k,  int M, int clusters_to_search, int points_to_rerank, std::string* filter_attributes, int n_filter_attributes, std::string filter_approach, std::string exact_search_approach, float* recall, int* approx_latency, int* exact_latency);
