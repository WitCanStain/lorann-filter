...
if __name__ == "__main__":
    ...
    for (idx, index_param_set) in enumerate(index_param_sets):
        ...
        for param_set in search_param_sets:
            if skip_rest and prev_filter_approach == param_set["filter_approach"]:
                continue
            elif prev_filter_approach != param_set["filter_approach"]:
                skip_rest = False
        ...
        experiment parameters:\nn_repeat_runs = {param_set["n_repeat_runs"]}\nn_query_indices = {len(param_set["query_indices"])}\n") #\nquery_indices = {param_set["query_indices"]}
            ...
            recall = recall.value
            approx_latency = approx_latency.value
            exact_latency = exact_latency.value
            filter_time = filter_time.value
            end_time = time.process_time()
            elapsed_time = end_time - start_time
